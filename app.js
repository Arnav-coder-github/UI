...
function tick() {
  ...
  // While we're waiting on the server, surface a live "elapsed" counter
  // in the header so the user sees the timer ticking even before the
  // first audio chunk lands. Reported in seconds because raw ms numbers
  // change too fast to read. Uses whichever timer is active: pending
  // (no gen_id yet) or the active gen's claimed start.
  if (aiState === "thinking") {
